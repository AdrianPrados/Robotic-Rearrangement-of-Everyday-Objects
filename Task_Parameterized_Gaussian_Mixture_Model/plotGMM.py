...
def Denman_Beavers_sqrtm(A):
    # To correct the problem with no positive matrtiz defined we applied a spectral descomposition
    # applying A=PDP^-1 where P is a matriz formed by the eigenvectors
    vals,vector = eigh(A)
    if vals[vals < 0]:
        vals[vals < 0] = 1e-6
        B = vector @ np.diag(vals) @ vector.T
        print("Nueva matriz: {}".format(B))
        A=B
    Y = A
    Z = np.eye(len(A))
    error = 1
...
def plotGMM(Mu, Sigma, color,display_mode, ax):
    ...
    if display_mode==1:
        ...
        for j in range (0,nbData):
            stdev= Denman_Beavers_sqrtm(0.05*Sigma[:,:,j]) #! By defect used 0.1
